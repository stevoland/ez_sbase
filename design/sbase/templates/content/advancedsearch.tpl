...
{if $use_template_search}
    ...
    {if or( $search_contentclass_id|not, $search_contentclass_id|lt(0) ) }
	    {def $exc_classes=-1 }
		{if ezini_hasvariable( 'Search', 'IncludeClasses', 'search.ini' )}
			{set $search_contentclass_id=ezini( 'Search', 'IncludeClasses', 'search.ini' )}
		{/if}
		{if or( $search_contentclass_id|is_array|not, $search_contentclass_id|count|not ) }
			{set $search_contentclass_id=-1 }
			...
			{if and( $exc_classes, $exc_classes|count )}
				...
				{set $search_contentclass_id=array()}
				{foreach $all_classes as $class}
					{if $exc_classes|contains($class.id)|not}
						{set $search_contentclass_id=$search_contentclass_id|append($class.id)}
					{/if}
				{/foreach}
			{/if}
		{/if}
		{if $search_sub_tree|count|not}
			{set $search_sub_tree=array(2)}
		{/if}
    {/if}
