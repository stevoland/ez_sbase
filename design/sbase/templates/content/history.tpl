...
<tr>
    ...
    {if $multilingual_site}<th>{'Modified translation'|i18n( 'design/ezwebin/content/history' )}</th>{/if}
...
<form action={concat( $module.functions.history.uri, '/', $object.id, '/' )|ezurl} method="post">
        ...
        {else}
        	{foreach $languages as $lang}
        <input type="hidden" name="Language" value="{$lang.locale}" />
        	{break}
            {/foreach}
        {/if}
...
<tr>
    <th>{'Version'|i18n( 'design/ezwebin/content/history' )}</th>
    {if $multilingual_site}<th>{"Translations"|i18n("design/ezwebin/content/history")}</th>{/if}
    <th>{'Creator'|i18n( 'design/ezwebin/content/history' )}</th>
    <th>{'Created'|i18n( 'design/ezwebin/content/history' )}</th>
    <th>{'Modified'|i18n( 'design/ezwebin/content/history' )}</th>
    {if $multilingual_site}<th class="tight">&nbsp;</th>{/if}
    <th class="tight">&nbsp;</th>
</tr>
