{* a hacked copy of design/admin/contentstricutemenu/content_structure_menu_dynamic.tpl
...
{if $ui_context|eq('browse') }
    {def $csm_menu_item_click_action='content/browse' }
{/if}
...
{literal}
        ...
        html += '&nbsp;<a class="nodetext" href="'
            + ( ( this.action )? this.action + '/' + item.node_id:
                                 {/literal}'{'/content/view/detail/'|ezurl(no)}{literal}/' + item.node_id )
            + '"';
...
{cache-block keys=$root_node_id expiry="0"}
    ...
    var rootNode = {ldelim}{*
        ...
        *}"modified_subnode":{$root_node.modified_subnode}{rdelim};
