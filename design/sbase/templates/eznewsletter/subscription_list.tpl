...
<div class="right">
        <form name="subscription_list" method="post" action={concat( '/newsletter/subscription_list/', $subscriptionList.url_alias )|ezurl} style="display:inline">
            ...
            <select name="statusFilter[]" {*multiple="multiple" size="8"*}>
                <option value="-1" {cond( $statusFilter|contains( -1 ), 'selected="selected"', '' )}>{'Show all'|i18n( 'design/eznewsletter/subscription_list' )}</option>
...
{*
            ...
            <label>{'VIP:'|i18n( 'design/eznewsletter/subscription_list' )}</label>
            <select name="(vipFilter)[]" multiple="multiple" size="8">
                <option value="-1" {cond( $view_parameters.statusFilter|contains( -1 ), 'selected="selected"', '' )}>{'Show all'|i18n( 'design/eznewsletter/subscription_list' )}</option>
                {foreach $vip_map as $value => $name}
                    <option value="{$value}" {cond( $view_parameters.vipFilter|contains( $value ), 'selected="selected"', '' )} >{$name|wash}</a>
                {/foreach}
            </select>
...
<tr class="{$seq}">
    <td><input type="checkbox" name="SubscriptionIDArray[]" value="{$subscriber.id}" title="{'Select subscriber for removal'|i18n( 'design/eznewsletter/subscription_list' )}" /></td>
    <td class="number" align="right">{$subscriber.id}</td>
    <td>{cond( $subscriber.user_id|gt(0), concat( '<a href=', $subscriber.user.contentobject.main_node.url_alias|ezurl, '>' ), '' )}{$subscriber.firstname|wash}{cond( $subscriber.user_id|gt(0), '</a>', '' )}</td>
    <td>{cond( $subscriber.user_id|gt(0), concat( '<a href=', $subscriber.user.contentobject.main_node.url_alias|ezurl, '>' ), '' )}{$subscriber.name|wash}{cond( $subscriber.user_id|gt(0), '</a>', '' )}</td>
    <td><a href="mailto:{$subscriber.email|wash}">{$subscriber.email|wash}</a></td>
{*    <td>{$vip_map[$subscriber.vip]|wash}</td> *}
    <td>{$status_map[$subscriber.status]|wash}</td>
    <td>{cond( $subscriber.created|gt(0), $subscriber.created|l10n( shortdatetime ), 'n/a'|i18n( 'design/eznewsletter/subscription_list' ) )}</td>
    <td>{cond( $subscriber.confirmed|gt(0), $subscriber.confirmed|l10n( shortdatetime ), 'n/a'|i18n( 'design/eznewsletter/subscription_list' ) )}</td>
    <td>{cond( $subscriber.approved|gt(0), $subscriber.approved|l10n( shortdatetime ), 'n/a'|i18n( 'design/eznewsletter/subscription_list' ) )}</td>
    <td>{cond( $subscriber.removed|gt(0), $subscriber.removed|l10n( shortdatetime ), 'n/a'|i18n( 'design/eznewsletter/subscription_list' ) )}</td>
    <td>
